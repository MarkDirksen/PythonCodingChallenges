...
FreeThrowsAttempted = np.array([KobeBryant_FTA,JoeJohnson_FTA,LeBronJames_FTA,CarmeloAnthony_FTA,DwightHoward_FTA,ChrisBosh_FTA,ChrisPaul_FTA,KevinDurant_FTA,DerrickRose_FTA,DwayneWade_FTA])


colors = ['black','green','yellow','grey','brown','red','purple','blue','magenta','orange']
def PlotStats(data,Players=Players):
 for name in Players:
   plt.plot(data[Pdict[name]],c=colors[Pdict[name]],ls='--',marker='s',ms='5',label=name)
 plt.legend(loc='upper left',bbox_to_anchor=(1,1))
 plt.xticks(list(range(0,10)),Seasons,rotation='vertical')
 plt.show()


PlotStats(FreeThrows/Games)
PlotStats(FreeThrows/FreeThrowsAttempted) 
PlotStats((Points-FreeThrows)/FieldGoals)